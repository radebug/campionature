...
Object.assign(els, {
  ...
  btnMobileCartToggle: $("#btnMobileCartToggle"),
  btnCloseMobileCart: $("#btnCloseMobileCart"),
...
/* --- Mobile cart drawer + shipment history canonical fixes --- */
...
function setMobileCartOpen(open) {
  ...
  if (els.mobileCartBackdrop) els.mobileCartBackdrop.hidden = !open;
}
...
function bindMobileCartControls() {
  ...
  window.addEventListener('resize', () => { if (!isMobileViewport()) setMobileCartOpen(false); });
}

const __oldWire = wire;
wire = function(){
  __oldWire();
