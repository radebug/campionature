...
/* === Canonical mobile cart + shipment history overrides === */
let __mobileCartBound = false;
...
function setMobileCartOpen(open) {
  const active = !!open && isMobileViewport();
  document.body.classList.toggle('mobile-cart-open', active);
  if (els.mobileCartBackdrop) els.mobileCartBackdrop.hidden = !active;
}

function bindMobileCartControls() {
  if (__mobileCartBound) return;
  __mobileCartBound = true;

  els.btnMobileCartToggle?.addEventListener('click', () => setMobileCartOpen(true));
  els.btnCloseMobileCart?.addEventListener('click', () => setMobileCartOpen(false));
  els.mobileCartBackdrop?.addEventListener('click', () => setMobileCartOpen(false));
  window.addEventListener('resize', () => {
    if (!isMobileViewport()) setMobileCartOpen(false);
  });
}

const __wireBase = wire;
wire = function(){
  __wireBase();
...
function normalizeShipmentRecords(rows) {
  ...
  return arr.map((s, index) => {
    ...
    return {
      id: s.id || `ship_${index}_${Date.now().toString(16)}_${Math.random().toString(16).slice(2,8)}`,
      date: s.date || todayISO(),
      destination: s.destination || "",
      recipient: s.recipient || "",
      reference: s.reference || "",
...
function ensureShipmentStateNormalized() {
  state.shipments = normalizeShipmentRecords(state?.shipments || []);
...
function filteredShipments() {
  const q = (els.shipHistSearch?.value || '').trim().toLowerCase();
  const arr = ensureShipmentStateNormalized().map((s, idx) => ({ ...s, _idx: idx }));
  arr.sort((a,b) => `${b.date}|${b.createdAt || ''}`.localeCompare(`${a.date}|${a.createdAt || ''}`));
  if (!q) return arr;
  return arr.filter(s =>
    [shipmentItemsText(s), s.destination, s.recipient, s.reference, s.notes, s.extraUE ? 'dhl extra ue' : '']
      .join(' ')
      .toLowerCase()
      .includes(q)
  );
...
function renderShipmentCart() {
  ...
  if (!shipmentCart.length) {
    box.innerHTML = `<div class="shipmentCartEmpty">Cart empty. Use “Add to cart” on a product.</div>`;
    if (els.shipmentCartSummary) els.shipmentCartSummary.textContent = "No products in cart.";
    if (els.mobileCartCount) els.mobileCartCount.textContent = '0';
    if (els.cartOverview) {
      els.cartOverview.innerHTML = `
        <div class="cartOverviewCard is-empty">
          <div class="cartOverviewStat"><b>0</b><span>lines</span></div>
  ...
  for (const item of shipmentCart) {
    ...
    const unitOptions = [`<option value="units" ${item.unitMode !== "ct" ? 'selected' : ''}>Units</option>`];
    if (p.ctSize && p.ctSize > 0) {
      unitOptions.push(`<option value="ct" ${item.unitMode === "ct" ? 'selected' : ''}>Boxes / CT</option>`);
    }
  ...
  if (els.mobileCartCount) els.mobileCartCount.textContent = String(shipmentCart.length);
  updateCartOverview(totalUnits, totalCt);
}
...
function renderShipmentHistory() {
  ...
  if (!rows.length) {
    ...
    for (const s of rows) {
      const tr = document.createElement('tr');
      tr.dataset.shipIndex = String(s._idx);
      tr.innerHTML = `
        <td>${formatDateDMY(s.date)}</td>
        <td>${escapeHtml(shipmentItemsText(s))}</td>
        <td>${escapeHtml(s.destination || '')}</td>
      ...
      tr.querySelector('[data-act="pdf"]')?.addEventListener('click', () => exportShipmentFromHistory(s._idx, 'pdf'));
      tr.querySelector('[data-act="edit"]')?.addEventListener('click', () => editShipment(s._idx));
      tr.querySelector('[data-act="dhl"]')?.addEventListener('click', () => exportShipmentFromHistory(s._idx, 'dhl'));
      tr.querySelector('[data-act="del"]')?.addEventListener('click', () => deleteShipment(s._idx));
      els.shipHistBody.appendChild(tr);
    }
  }

  if (els.shipHistCount) {
    els.shipHistCount.textContent = `${rows.length} shipment${rows.length === 1 ? '' : 's'}`;
  }
}

function getShipmentByIndex(index) {
  const rows = ensureShipmentStateNormalized();
...
function editShipment(index) {
  if (!isAdmin()) return;
  const idx = Number(index);
  const shipment = getShipmentByIndex(idx);
  if (!shipment) return;
  if (!confirm('Load this shipment back into the cart for editing?')) return;
  ...
  if (isMobileViewport()) setMobileCartOpen(true);
  document.getElementById('shipmentCartPanel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function exportShipmentFromHistory(index, kind = 'pdf') {
  const shipment = getShipmentByIndex(index);
  if (!shipment) return;
  if (kind === 'pdf') exportShipmentDraftPDF(shipment);
  else if (kind === 'excel') exportShipmentDraftExcel(shipment);
  else if (kind === 'dhl') exportDHLList(shipment);
